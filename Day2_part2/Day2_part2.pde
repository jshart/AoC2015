...
String filebase = new String("C:\\Users\\jsh27\\OneDrive\\Documents\\GitHub\\AoC2015\\Day2_part1\\data\\mydata");
...
void setup() {
  ...
  int masterTotal=0;
  int s1=0,s2=0,s3=0;
  int ribbonForPresent=0;
  int bowForPresent=0;

  // Loop through each input item...
  for (i=0;i<input.lines.size();i++)
  {
    strings=input.lines.get(i).split("x");
    m=new Minimum();
    total=0;
    nums.clear();
    ...
    for (j=0;j<3;j++)
    {
      nums.append(Integer.parseInt(strings[j]));
    }
    nums.sort();
    ribbonForPresent=(nums.get(0)*2) + (nums.get(1)*2);
    bowForPresent=nums.get(0)*nums.get(1)*nums.get(2);

    
    println(nums.get(0)+" x "+nums.get(1)+" x "+nums.get(2)+"=["+ribbonForPresent+"]["+bowForPresent+"]["+(ribbonForPresent+bowForPresent)+"]");
    masterTotal+=(ribbonForPresent+bowForPresent);
  }
  println("Master total:"+masterTotal);
}
